...
function codes_editor_area() {
	...
	// Create nonce.
	wp_nonce_field( 'custom-codes-save', 'custom-codes-save_' . $post->ID );
	...
	<div id="codes_editor_area" :class="{ loaded: initialized, fullscreen: fullscreen, loading: loading }" style="opacity: 0;">
		...
		<div id="topbar">
			<div class="left">
				...
				<select name="language" v-model="selectedLangID" @change="switchLang">
					<option value="" selected disabled><?php esc_html_e( 'SELECT EDITOR TYPE', 'custom-codes' ); ?></option>
					<optgroup :label="langGroup.name" v-for="langGroup in langGroups" :key="langGroup.id">
						<option :value="lang.id" v-for="lang in langs.filter(lang => lang.group == langGroup.id)" :key="lang.id" :disabled="lang.pro" v-if="!lang.pro">{{ lang.name }}{{ lang.pro ? " (PRO)" : "" }}</option>
					</optgroup>
				</select>
				...
				<div class="indicators">
					...
					<span class="tooltip-not-contained bottom-tooltip" v-if="loading" data-tooltip="<?php esc_html_e( 'Saving...', 'custom-codes' ); ?>">
					...
					<span class="dashicons dashicons-yes bottom-tooltip" style="color: white;" :data-tooltip="'Last Process Time: ' + processTime" v-else-if="processTime"></span>
			...
			<div class="right">
				...
				<a href="#" @click.prevent class="tooltip-sub tooltip-not-contained bottom-tooltip tooltip-focus" data-tooltip>
					<span class="tooltip shortcuts">
						<strong class="title"><?php esc_html_e( 'SHORTCUTS', 'custom-codes' ); ?></strong> <br>
						<div class="options">
							<b><?php esc_html_e( 'Save', 'custom-codes' ); ?>: </b> Command/Ctrl + S <br>
							<b><?php esc_html_e( 'Find', 'custom-codes' ); ?>: </b> Command/Ctrl + F <br>
							<b><?php esc_html_e( 'Multiple Lines', 'custom-codes' ); ?>: </b> <?php esc_html_e( 'Option/Alt + Click and Drag', 'custom-codes' ); ?> <br>
							<b><?php esc_html_e( 'Add Multi Cursor', 'custom-codes' ); ?>: </b> <?php esc_html_e( 'Command/Ctrl + Click', 'custom-codes' ); ?> <br>
							<b><?php esc_html_e( 'Comment the Line', 'custom-codes' ); ?>: </b> Command/Ctrl + 7 <br>
							<b><?php esc_html_e( 'Tidy Codes', 'custom-codes' ); ?>: </b> Command/Ctrl + 8 <br>
							<b><?php esc_html_e( 'Toggle Fullscreen Mode', 'custom-codes' ); ?>: </b> Command/Ctrl + G <br>
							<b><?php esc_html_e( 'Space Hierarchy', 'custom-codes' ); ?>: </b> <?php esc_html_e( '(Select) + Shift + Tab', 'custom-codes' ); ?> <br>
							<b><?php esc_html_e( 'Emmet Abbreviations', 'custom-codes' ); ?>: </b> <?php esc_html_e( 'Write Abbs. + Tab', 'custom-codes' ); ?> <br>
						</div>
					</span>
					<?php echo wp_kses( $wp_filesystem->get_contents( CODES_PLUGIN_DIR . '/assets/image/icon-question.svg' ), codes_svg_args() ); ?>
				</a>

				<div class="subtabs" v-if="activeEditor">
					<button
						v-for="editor in currentLang.editors"
						:class="{
							active: activeEditor.id == editor.id,
							saved: isEditorSaved('editor-' + editor.id),
							disabled: editor.id.endsWith('-body-opening') && location != 'frontend'
						}"
					...
					>
						<span class="tooltip" style="text-align: center;" v-if="editor.description">
							{{ editor.description }}
							<div v-if="currentLangGroup.id == 'style'">{{ getMediaQueryText(editor.id) }}</div>
						</span>
				...
				<a href="#" @click.prevent="toggleFullScreen" v-if="!fullscreen" class="tooltip-not-contained left-tooltip" data-tooltip="Toggle Fullscreen (Cmd/Ctrl G)"><?php echo wp_kses( $wp_filesystem->get_contents( CODES_PLUGIN_DIR . '/assets/image/icon-fullscreen.svg' ), codes_svg_args() ); ?></a>
				<a href="#" @click.prevent="toggleFullScreen" v-if="fullscreen" class="tooltip-not-contained left-tooltip" data-tooltip="Toggle Fullscreen (Cmd/Ctrl G)"><?php echo wp_kses( $wp_filesystem->get_contents( CODES_PLUGIN_DIR . '/assets/image/icon-fullscreen-exit.svg' ), codes_svg_args() ); ?></a>

			</div>
		</div>

		<div class="editors" :class="{ spacing: currentLang.id == 'css' || currentLang.id == 'js' }" v-if="currentLang">

			<div class="editor-addition before CodeMirror" :class="'cm-s-'+ theme" v-if="currentLang.id == 'css' || currentLang.id == 'js'">
				<span>&lt;{{ currentLangGroup.id }} type=&quot;{{ currentLangGroup.mode }}&quot;&gt;</span>
			</div>
			...
			<div class="editor" v-show="'<?php echo esc_js( $editor_id ); ?>' == activeEditor.id && !outputOpen" :style="'font-size: ' + fontSize + 'px;'" <?php echo $editor_exists ? 'exists' : ''; ?> <?php echo $editor_writable ? 'writable' : ''; ?>>

				<div class="codes-debug" style="display: nonee;">(Editor Exists: <?php echo $editor_exists ? 'Yes' : 'No'; ?> | Readable: <?php echo codes_is_readable( $file_directory ) ? 'Yes' : 'No'; ?> | Writable: <?php echo codes_is_writable( $file_directory ) ? 'Yes' : 'No'; ?> | CHMOD: <?php echo esc_html( codes_chmod_check( $file_directory ) ); ?>)</div>

				<div class="codes-debug" style="display: nonee;">(Codes Exists: <?php echo $editor_exists ? 'Yes' : 'No'; ?> | Readable: <?php echo $editor_readable ? 'Yes' : 'No'; ?> | Writable: <?php echo $editor_writable ? 'Yes' : 'No'; ?> | CHMOD: <?php echo esc_html( codes_chmod_check( $file_directory ) ); ?>)</div>

				<textarea :id="'editor-' + '<?php echo esc_js( $editor_id ); ?>'" :name="'editor-' + '<?php echo esc_js( $editor_id ); ?>'" :disabled="editorActive('<?php echo esc_js( $editor_id ); ?>')" placeholder="<?php echo esc_attr( $editor_placeholder ); ?>" <?php echo ! $editor_writable ? 'readonly' : ''; ?>><?php echo esc_textarea( $editor_content ); ?></textarea>
			</div>
			...
			<div class="editor-addition after CodeMirror" :class="'cm-s-'+ theme" v-if="currentLang.id == 'css' || currentLang.id == 'js'">
				<span>&lt;/{{ currentLangGroup.id }}&gt;</span>
			</div>
		...
		<div class="lang-notice" v-if="currentLang && currentLang.id != postLanguage && postLanguage != ''">
			...
			<?php esc_html_e( 'Please click "Update" to confirm the new language.', 'custom-codes' ); ?>
		</div>
		<input type="hidden" id="ajax-saver" name="codes_doing_ajax" :value="1" v-else-if="ajaxSave && currentLang && postName != ''">

		<div id="bottombar" v-if="(currentLang && currentLang.id == '<?php echo esc_js( $current_language ); ?>') || ('<?php echo esc_js( $current_language ); ?>' == '' && currentLang)">
			<div class="left">

				<select name="theme" v-model="theme" @change="switchTheme">
					<option value="dark" selected><?php esc_html_e( 'Dark Theme', 'custom-codes' ); ?></option>
					<option value="default"><?php esc_html_e( 'Light Theme', 'custom-codes' ); ?></option>
				...
				<label>
					<?php esc_html_e( 'Font Size:', 'custom-codes' ); ?>
			...
			<div class="right">
				...
				<button @click.prevent="toggleOutput" :class="{ active: outputOpen }" v-if="currentLang.output && currentLang && currentLang.id == '<?php echo esc_js( $current_language ); ?>'"><?php esc_html_e( 'OUTPUT', 'custom-codes' ); ?></button>
				<button @click.prevent="jQuery('#publish').trigger('click')" class="save tooltip-not-contained" data-tooltip="Cmd/Ctrl + S" :disabled="loading ? true : null" v-if="fullscreen"><?php esc_html_e( 'SAVE', 'custom-codes' ); ?></button>
